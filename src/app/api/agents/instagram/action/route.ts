...
export async function POST(request: Request) {
  ...
  const { companyId, opportunityId, action, scheduledDate, customDraft } = body;
  ...
  if (action === "publish") {
    return Response.json({ error: "Publishing is not connected for Instagram. Mark the draft ready or add a manual planning date instead." }, { status: 409 });
  }
  ...
  const newFeedback = [
    ...feedbackLog,
    {
      ...
      details: { scheduledDate, customDraft },
    },
  ];
